...
const form = useForm({
  assessment_date: new Date().toISOString().substr(0, 10),
  division_risk_id: '',
  likelihood_level: '',
  impact_level: '',
  notes: ''
...
const selectedDivisionRisk = ref<number | null>(null)
...
// ส่งข้อมูลการประเมิน
const submitForm = () => {
  form.post(route('risk-assessments.store'), {
    onSuccess: () => {
...
<template>
  <AppLayout>
    <Head title="สร้างการประเมินความเสี่ยงใหม่" />
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-2 mb-6">
        ...
        <h1 class="text-2xl font-semibold text-gray-900">สร้างการประเมินความเสี่ยงใหม่</h1>
      </div>

      <Card class="max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle>แบบฟอร์มประเมินความเสี่ยง</CardTitle>
          <CardDescription>กรอกข้อมูลเพื่อประเมินความเสี่ยงตามเกณฑ์การประเมินที่กำหนด</CardDescription>
        </CardHeader>
        <CardContent>
          <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div>
              ...
              <Select @update:model-value="onDivisionRiskChange">
                <SelectTrigger 
                  class="w-full mt-1" 
